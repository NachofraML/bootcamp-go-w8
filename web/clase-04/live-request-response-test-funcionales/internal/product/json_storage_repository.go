package product

import (
	"errors"
	"fmt"
	"github.com/NachofraML/bootcamp-go-w8/web/clase-04/live-request-response-test-funcionales/internal/domain"
	"github.com/NachofraML/bootcamp-go-w8/web/clase-04/live-request-response-test-funcionales/pkg/store"
)

func NewJsonStorageRepository(jsonStoragePath string) Repository {
	jsonStorage, err := store.NewJsonStorage(jsonStoragePath)
	if err != nil {
		panic(err)
	}
	return &JsonStorageRepository{jsonStorage}
}

type JsonStorageRepository struct {
	jsonStorage *store.JsonStorage
}

func (sr JsonStorageRepository) GetAll() ([]*domain.Producto, error) {
	return sr.jsonStorage.GetAll()
}

func (sr JsonStorageRepository) GetByID(id int) (producto *domain.Producto, err error) {
	producto, err = sr.jsonStorage.GetProductById(id)
	if err != nil {
		if errors.Is(err, store.ErrJsonStorageProductNotFound) {
			err = store.ErrJsonStorageProductNotFound
			return
		}
		if errors.Is(err, store.ErrJsonStorageCantOpenFile) {
			err = store.ErrJsonStorageCantOpenFile
			return
		}
		return
	}
	return
}

func (sr JsonStorageRepository) Save(producto *domain.Producto) (lastInsertId int, err error) {
	lastInsertId, err = sr.jsonStorage.CreateProduct(producto)
	if err != nil {
		return
	}
	return
}

func (sr JsonStorageRepository) Update(id int, producto *domain.Producto) (err error) {
	err = sr.jsonStorage.UpdateProduct(id, producto)
	if err != nil {
		fmt.Printf("repository json: %e", err)
		return
	}
	return
}

func (sr JsonStorageRepository) Delete(id int) (err error) {
	err = sr.jsonStorage.DeleteProduct(id)
	if err != nil {
		return
	}
	return
}
